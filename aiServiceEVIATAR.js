...
function updateContext(interaction) {
  // שלב 1: הוסיפו timestamp לאינטראקציה
  interaction.timestamp = new Date().toISOString(); // check why we need timestamp?

  // שלב 2: טענו את ההיסטוריה הקיימת
  const history = loadAIHistory();

  // שלב 3: הוסיפו את האינטראקציה החדשה למערך
  history.push(interaction);// instead of push, decide which text to put in history and what to remove - let the AI decide

  // שלב 4: שמרו בחזרה ל-localStorage
  localStorage.setItem(AI_HISTORY_KEY, JSON.stringify(history));

  // שלב 5: הדפיסו הודעת אישור
...
function loadAIHistory() {
  // the job of this function id to save the history to JSON file
  // שלב 1: קראו מ-localStorage
  const saved = localStorage.getItem(AI_HISTORY_KEY);

  // שלב 2: אם אין כלום — החזירו מערך ריק
  if (!saved) return [];

  // שלב 3: פרסו ל-JSON והחזירו
